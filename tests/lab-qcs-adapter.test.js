...
describe('QCS lab adapter', () => {
  ...
  test('listOrders fetches a single page of QCS orders with the bearer token', async () => {
    const qcs = require('../src/functions/lab/lib/adapters/qcs');
    qcs.clearTokenCache();
    const calls = [];
    const transport = {
      ...
      async get(url, options) {
        calls.push({ method: 'GET', url, params: options.params, headers: options.headers });
        return {
          data: {
            data: [{ id: 'QCS-1', member: { mobile: '13800000001' }, progress: 'complete' }],
    ...
    const result = await qcs.listOrders({
      config: { api_base_url: 'https://qcs.example/third-party', api_key: 'client-id', api_secret: 'client-secret', transport },
    });

    assert.deepEqual(result, [{ id: 'QCS-1', member: { mobile: '13800000001' }, progress: 'complete' }]);
    assert.equal(calls[1].method, 'GET');
    assert.equal(calls[1].url, 'https://qcs.example/third-party/services/labtest/orders');
    assert.equal(calls[1].params.page, 1);
    assert.equal(calls[1].headers.Authorization, 'Bearer token');
  });

  test('listOrders paginates across all pages and forwards progress filter', async () => {
    const qcs = require('../src/functions/lab/lib/adapters/qcs');
    qcs.clearTokenCache();
    const pages = {
      1: { data: [{ id: 'QCS-1' }, { id: 'QCS-2' }], meta: { pagination: { current_page: 1, total_pages: 3 } } },
      2: { data: [{ id: 'QCS-3' }], meta: { pagination: { current_page: 2, total_pages: 3 } } },
      3: { data: [{ id: 'QCS-4' }], meta: { pagination: { current_page: 3, total_pages: 3 } } },
    };
    const seenPages = [];
    const seenProgress = [];
    const transport = {
      async post() { return { data: { access_token: 'token' } }; },
      async get(url, options) {
        seenPages.push(options.params.page);
        seenProgress.push(options.params.progress);
        return { data: pages[options.params.page] };
      },
    };
    ...
    assert.deepEqual(result.map(o => o.id), ['QCS-1', 'QCS-2', 'QCS-3', 'QCS-4']);
    assert.deepEqual(seenPages, [1, 2, 3]);
    assert.deepEqual(seenProgress, ['complete', 'complete', 'complete']);
  });

  test('phoneFromOrder prefers mobile, then primary_mobile, then telephone', () => {
    const qcs = require('../src/functions/lab/lib/adapters/qcs');

    assert.equal(qcs.phoneFromOrder({ member: { mobile: '13800000001', telephone: '4000330187' } }), '13800000001');
    assert.equal(qcs.phoneFromOrder({ member: { mobile: null, primary_mobile: '13900000002', telephone: '4000330187' } }), '13900000002');
    assert.equal(qcs.phoneFromOrder({ member: { mobile: null, primary_mobile: null, telephone: '4000330187' } }), '4000330187');
    assert.equal(qcs.phoneFromOrder({ member: {} }), '');
    assert.equal(qcs.phoneFromOrder({}), '');
    assert.equal(qcs.phoneFromOrder({ data: { member: { mobile: '13700000003' } } }), '13700000003');
  });
