...
describe('BiomarkerEstimator - determinism', () => {
    test('same seed → identical BiomarkerValues across 100 runs', () => {
        ...
        const runs = [];
        for (let i = 0; i < 100; i++) {
            const e = new BiomarkerEstimator(...baseInputs(), [], { seed, persistentSeed: seed });
    ...
    test('different seeds → different outputs (very likely)', () => {
        const a = new BiomarkerEstimator(...baseInputs(), [], { seed: 'user-a:2026-05-01', persistentSeed: 'user-a:2026-05-01' }).generateReport().BiomarkerValues;
        const b = new BiomarkerEstimator(...baseInputs(), [], { seed: 'user-b:2026-05-01', persistentSeed: 'user-b:2026-05-01' }).generateReport().BiomarkerValues;
...
describe('BiomarkerEstimator - tag adjustments', () => {
    ...
    const options = { seed, persistentSeed: seed };

    test('inflammation_load_high pushes hsCRP and IL6 strictly higher than baseline', () => {
        const baseline = new BiomarkerEstimator(...baseInputs(), [], options).generateReport().BiomarkerValues;
        const tagged   = new BiomarkerEstimator(...baseInputs(), ['inflammation_load_high'], options).generateReport().BiomarkerValues;
    ...
    test('inflammation_load_low pushes hsCRP and IL6 strictly lower than baseline', () => {
        const baseline = new BiomarkerEstimator(...baseInputs(), [], options).generateReport().BiomarkerValues;
        const tagged   = new BiomarkerEstimator(...baseInputs(), ['inflammation_load_low'], options).generateReport().BiomarkerValues;
        assert.ok(tagged.hsCRP < baseline.hsCRP, `hsCRP ${tagged.hsCRP} not < ${baseline.hsCRP}`);
        assert.ok(tagged.IL6   < baseline.IL6,   `IL6 ${tagged.IL6} not < ${baseline.IL6}`);
    });
    ...
    test('diabetes_diagnosed bumps GA upward; Chinese alias works the same', () => {
        const baseline = new BiomarkerEstimator(...baseInputs(), [], options).generateReport().BiomarkerValues;
        const english  = new BiomarkerEstimator(...baseInputs(), ['diabetes_diagnosed'], options).generateReport().BiomarkerValues;
        const chinese  = new BiomarkerEstimator(...baseInputs(), ['糖尿病'], options).generateReport().BiomarkerValues;
    ...
    test('unknown tags are silently dropped (no throw, no effect)', () => {
        const baseline = new BiomarkerEstimator(...baseInputs(), [], options).generateReport().BiomarkerValues;
        const noisy    = new BiomarkerEstimator(...baseInputs(), ['nonsense_tag', 'another'], options).generateReport().BiomarkerValues;
        assert.deepStrictEqual(noisy, baseline);
    });

    test('AppliedTags reflects normalized + filtered set', () => {
        const report = new BiomarkerEstimator(...baseInputs(), ['糖尿病', 'unknown', 'inflammation_load_high'], options).generateReport();
        assert.deepStrictEqual(report.AppliedTags.sort(), ['diabetes_diagnosed', 'inflammation_load_high'].sort());
    });

    test('test_data overrides skip tag adjustments (measured value wins if in [0.2, 2.5])', () => {
        const measured = { hsCRP: 1.5 };
        const report = new BiomarkerEstimator(40, measured, { Weight: 70, Height: 175 }, ['inflammation_load_high'], options).generateReport();
        assert.strictEqual(report.BiomarkerValues.hsCRP, 1.5);
    });

    test('hsCRP < 0.2 is ignored and estimated instead', () => {
        const measured = { hsCRP: 0.1 };
        const report = new BiomarkerEstimator(40, measured, { Weight: 70, Height: 175 }, [], options).generateReport();
        assert.notStrictEqual(report.BiomarkerValues.hsCRP, 0.1);
        assert.ok(report.BiomarkerValues.hsCRP > 0.1, 'Should have estimated a more likely value');
    });
