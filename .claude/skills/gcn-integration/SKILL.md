...
GCN sets its own `partners.partner_type` directly to the real nano tier key (`light_entrepreneur`/`leader_partner`/`operations_center`, already-seeded `partner_types` rows; since 2026-09-16 also `diamond_store` / 大区联盟中心, a fourth tier at 70% wholesale discount that carries the same key on both sides — nano mirror row via `migration_partner_types_diamond_store.sql`, GCN via `migration_0118_diamond_store_tier.sql`; it is house-assigned, not offered on `partner-apply.html`) at provisioning time — it no longer hardcodes `partner_type='store'` with the real tier stashed in `metadata.nano_tier`. Tier/status changes on nano's side take effect the moment an admin next saves or deactivates the partner in nano's Partners tab (not automatically on GCN login, since login no longer touches nano) — see the 2026-07-29 fix below for how that re-sync actually reaches GCN.
