...
EXCLUDED_DEFAULT = [
 ('DOT-N6 / N9（尿石素 A / NMN）', '依据仅为参考估算的「细胞年龄」；无 GDF-15、NAD+ 实测'),
 ('DOT-N11 / N15 / N17（小檗碱 / 抗糖化 / 血脂）', '无血糖、HbA1c、血脂实测；小檗碱与降糖降脂药有相互作用，用药情况不明时不给'),
 ('DOT-N7（漆黄素脉冲）', '无任何衰老细胞负荷指标'),
 ('DOT-N2（D3 + K2 + 锌）', '维生素 D 未测；测了再定'),
]
LIMITS_DEFAULT = '① 目前没有正式化验报告，本报告的「现状」判断以自述与穿戴数据为主，均需复查证实；② 消费级戒指 / 手环的心率、HRV、SpO₂ 未经医用设备验证，只使用同设备内的分布与趋势；③ Kino 面板除 hsCRP 外为参考估算值，本报告不引用其数值作结论；④ 对话中的自述未经核实。'
...
def minimal(name, birth, sex, height, weight, conds, role, first_scan, n_scans, hs_readings, chat_lines, self_reports, extra_problems, tests_extra=(), prio_extra=(), d30_extra=(), created='2026-05', wearable=None, ba=None, other_zh=''):
    ...
    d['GUIDE_SUB'] = f'这份报告把您的全部记录——入门问卷、{n_scans} 次 Kino 扫描的原始读数、{chat_lines} 条对话' + ('、穿戴设备记录' if wearable else '') + '——放在一起阅读，并如实说明哪些是测量、哪些是估算。'
    ...
    d['CONFLICTS'] = f'<b>② 目前还没有正式化验、影像或体检报告</b>；问卷勾选的「{cz}」是自述，没有对应的数值。<b>③</b> 因此本报告不给出针对性的 Dots 剂量——只给出一张本月可以完成的检测清单。'
    d['PART1_DESC'] = f'先看结论：目前真正测量过的项目还很少——问卷里勾选的「{cz}」还只是自述。这份报告的全部价值，在于把它变成一张本月能完成的清单。'
    d['SUMMARY_SUB'] = '综合全部记录得出的核心判断'
    d['STATS'] = [(f'{age:g}<small style="font-size:11pt"> 岁</small>', '实际年龄', f'{s} · BMI {bmi}' if bmi else s, C['blue']), (str(n_scans), 'Kino 扫描', hs_readings, C['warning']), ('0', '正式化验 / 报告', '尚未上传', C['critical']), (str(chat_lines), '对话消息', '自述来源', C['muted'])]
    d['OVERALL'] = f'您 {age:g} 岁，{s}，BMI {bmi}。入门问卷勾选：<b>{cz}</b>。{n_scans} 次 Kino 扫描中只有 hsCRP 一项得到读数（{hs_readings}），其余五项与 App 里的生物年龄为按年龄和 BMI 填充的参考估算值——所以本报告不据此判断「哪一项偏高」，也不据此推荐配方。能负责任说的是：您的年龄段最值得先做的检查是什么、以及在结果回来之前哪些事不需要数据也成立。'
    ...
    d['DOCTOR'] = dict(profile=[('姓名', f'{name} · {s} · {birth} · {age:g} 岁'), ('体格', f'{height} cm · {weight} kg · BMI {bmi}（自述）'), ('既往（问卷）', cz), ('用药 / 补剂', '未记录'), ('自述', '；'.join(x[1] for x in self_reports[:3]) if self_reports else '无')],
        problems=list(extra_problems) + [f'问卷自述「{cz}」，尚无对应化验。→ 基础面板见下。', f'Kino {n_scans} 次扫描均未获得有效面板（{hs_readings}）；App 生物年龄 {ba or "—"} 为参考估算值。'],
    ...
    d['PLAN_PAGES'] = []
    d['DOTS'] = dict(sub='结果回来之前不给剂量 · 已持有的药筒先不开封', principle='配方库里每一款都对应一个可测的指标；您目前没有任何一项被测过，因此不以参考估算面板开方。下面是检测结果回来后的决策规则。', am=[], pm=[],
    ...
    d['SUMMARY_TXT'] = f'{name}，这是一份 {{PAGES}} 页的健康数据基线报告。先说明数据：目前属于您的实测数据只有问卷里的身高体重与自述「{cz}」；{n_scans} 次 Kino 扫描的原始读数在仪器有效窗口外（{hs_readings}），App 里的生物年龄 {ba or "—"} 为按年龄和 BMI 填充的参考估算值。所以报告没有给配方，而是给了一张本月能完成的检测清单（一次空腹抽血 + 7 天家庭血压），以及结果回来后决定 Dots 的规则。' + ('报告第一篇另有针对您自述问题的具体建议。' if extra_problems else '')
    return d
