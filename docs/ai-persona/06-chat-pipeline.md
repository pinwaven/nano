...
11. If not forked: runs either `runAgenticTurn()` (for the 5 high-risk intents) or an older 4-iteration `query_database` generic-SQL tool loop (non-agentic intents) — the latter's model-authored SQL is explicitly blocked from querying `biomarkers` directly.
