...
/**
 * List QCS orders for the third-party agent, following pagination until all
 * pages are collected. Mirrors listSampleCenters for auth/token handling.
 *
 * QCS returns { data: [...], meta: { pagination: { current_page, total_pages } } }.
 * Optional params.progress filters by status ('tobepaid' | 'tobeconfirmed' |
 * 'processing' | 'complete' | 'close').
 *
 * @param {object} args.config - { api_base_url, api_key, api_secret, cache }
 * @param {object} [args.params] - { page, progress }
 * @returns {Promise<Array>} all order summaries across pages
 */
async function listOrders({ config, params = {} }) {
    const transport = config.transport || axios;
    const baseUrl = trimTrailingSlash(config.api_base_url);
    const token = await getAccessToken(baseUrl, config, transport);
    ...
    // Hard cap guards against a malformed pagination response looping forever.
    const MAX_PAGES = 10000;
    for (let i = 0; i < MAX_PAGES; i += 1) {
        const res = await requestQcs(transport, 'GET', `${baseUrl}/services/labtest/orders`, null, {
            headers: authHeaders(token),
            timeout: 15000,
            params: { page, ...(params.progress ? { progress: params.progress } : {}) },
            stage: 'orders/list',
        });
        const body = res.data || {};
        const pageOrders = Array.isArray(body.data) ? body.data : [];
        orders.push(...pageOrders);

        const pagination = body.meta?.pagination;
        const totalPages = Number(pagination?.total_pages) || 1;
        const currentPage = Number(pagination?.current_page) || page;
        if (!pagination || currentPage >= totalPages || pageOrders.length === 0) break;
        page = currentPage + 1;
    }
    return orders;
}
...
/**
 * Extract the patient phone from a QCS order detail. QCS member objects expose
 * mobile / primary_mobile / telephone (any may be null). Accepts either a raw
 * order object or a { data: order } envelope. Returns '' when none present.
 */
function phoneFromOrder(detail) {
    const member = detail?.member || detail?.data?.member || {};
    return member.mobile || member.primary_mobile || member.telephone || '';
}
