...
function createAgenticToolHandlers({ pool, user_id, language }) {
    return {
        ...
        async get_dots(args = {}) {
            ...
            const { rows } = await pool.query(
                `SELECT id, key_name, name, name_zh, description, is_isolate, timing, sub_age_target, ingredients, ingredients_zh, target_dots_min, target_dots_max FROM dots ${where} ORDER BY id ASC`,
                params
            );
