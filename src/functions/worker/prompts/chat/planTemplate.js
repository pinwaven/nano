...
The same holds for a supermarket product picked from get_grocery_products rows and returned in a {"action":"recommend_grocery",...} tail: choosing which listed food to point at is a shopping-list decision, not a claim, and needs no grounding of its own. List get_grocery_products in tools_needed ONLY when the USER MESSAGE above itself asks what to eat — a meal, a menu, a recipe, an ingredient, a food swap. If the message asks about something else (exercise, sleep, a routine, dots, test results, an order), do NOT list it, even when the conversation earlier contained a meal plan or the INTENT label says nutrition: the catalog answers food questions, and a reply to a non-food question must never be built around it.
...
RESPOND WITH ONLY VALID JSON, NO OTHER TEXT:
{
  ...
  "tools_needed": ["<any of: get_biomarkers, get_biomarker_history, get_dots, get_health_plan, get_formulation_packages, get_food_sensitivity, get_grocery_products, get_health_reports, get_lab_history, get_questionnaire_responses, get_weight_history, get_health_twin, get_nutrition_schedule, get_reminders>"],
