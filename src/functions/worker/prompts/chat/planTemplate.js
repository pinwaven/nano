...
The same holds for a supermarket product picked from get_grocery_products rows and returned in a {"action":"recommend_grocery",...} tail: choosing which listed food to point at is a shopping-list decision, not a claim, and needs no grounding of its own. If the reply will recommend specific foods to eat and a grocery catalog is available (the system prompt says so), list get_grocery_products in tools_needed so the foods can be looked up before the reply is written.
