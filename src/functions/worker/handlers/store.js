...
// The nano store and the Neo dispenser: store items and per-channel inventory, SKUs, orders (single
// and batch, with partner discounts and commission recording; listing for the admin panel and the
// user), and the user's physical dot cartridges (insert / remove / dispense). None of this touches a
// formulation — that is handlers/dots.js — and the aeviva/waven storefront itself lives in the GCN
// sibling repo (CLAUDE.md §19); this is the in-app store the miniapp Store tab and the admin panel's
// Store/Inventory tabs talk to.
//
// The item/SKU CRUD and the order listings were always here; the order placement, channel
// inventory and cartridge handlers moved in from handlers/dots.js on 2026-09-16, unchanged.
...
module.exports = {
    // store items / SKUs / order listings
    handleGetOrders,
    handleGetMyOrders,
    handlePostStoreItem,
    handlePutStoreItem,
    ...
    handlePutSku,
    handleDeleteSku,
    // storefront, channel inventory, order placement (from handlers/dots.js)
    handleGetStoreItems,
