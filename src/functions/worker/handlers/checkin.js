...
const systemDailyCheckinTemplate = require('../prompts/viva/systemDailyCheckin');
...
/**
 * Dispatched by the dispatcher's Viva daily-check-in scan (source: 'acs.dispatcher',
 * type: 'checkin.daily') — fires on a user's own first app-open within whichever
 * time-of-day period (morning/midday/evening) is currently active. Deliberately lightweight:
 * a single completion, no PLAN/GENERATE/JUDGE agentic loop, matching the routine/low-stakes
 * nature of a daily check-in going out to every eligible user up to 3x/day.
 */
async function handleDailyCheckinEvent({ user_id, period }) {
    if (!user_id || !period) {
    ...
    try {
        ...
        const essentialKnowledge = await getEssentialBlock('viva');

        const systemPrompt = systemDailyCheckinTemplate({
        ...
        await saveChatMessage(user_id, 'ai', message, null, 'viva');
        await pool.query(
            'INSERT INTO notifications (user_id, notification_type, content, status) VALUES ($1, $2, $3, $4)',
            [user_id, `${period}_checkin`, message, 'pending']
        );
        ...
        // morning) will naturally try again via the dispatcher's own idempotency check.
