...
- **A dimension is only ever explained by its own biomarkers** · 2026-09-23
  ...
  - **Check** (`lib/factCheck.js`): a markdown heading naming one dimension scopes the lines under it, and inside that section 结合/提示/影响/机制/相关 with another dimension's marker counts; outside a dimension section soft words still don't (the false-positive guard stays). A sentence that denies the link ("并非由 hsCRP 驱动") is never flagged. More cause verbs (引起/诱发/所致/源自/due to).
  - **Last resort**: a misattributing sentence that survives both REVISE rounds is removed from the final reply in code (`stripDimensionMisattributions`, logged as `dimension_misattribution_stripped`) — REVISE sometimes rephrased the link instead of deleting it. Applies to every agentic turn, chat included.
  - Verified on dev: the check flags exactly the four bad saved reports and none of the clean ones; five fresh reports after the change had no misattribution. Tests in `tests/judge-strictness.test.js` and `tests/health-advice-subage-direction.test.js`.

- **Health advice no longer calls a younger sub-age "偏高"** · 2026-09-23
  - A report said 抗压年龄 was 明显偏高 in its summary while its own breakdown said 43.9 岁, 7.1 years younger than the user's 51. The prompt gave each sub-age as a bare number next to a marker tagged 偏高 (hsCRP), and left the model to work out the direction — which it got wrong whenever a younger dimension contained an elevated marker. Both personas, any user.
  - The direction is now computed in code (`relationToChrono` in `lib/subAgeLabels.js`) and stated in both health-advice templates: each sub-age line says "比实际年龄年轻 7.1 岁，该维度表现良好" / "老于…，偏高、需关注", a verdict line lists the elevated and the doing-well dimensions, and a rule says only an older dimension may be called elevated — an elevated marker inside a younger dimension is "worth watching", not an elevated dimension. Nano's English template also dropped the ambiguous "ahead / on-track / lagging".
  - Verified on dev with three fresh reports for the same user: all three summaries name exactly the two older dimensions as elevated. Tests: `tests/health-advice-subage-direction.test.js`.

- **Managed customers for SuperiorMed: coaches create and run login-less customer accounts** · 2026-09-23
  - SuperiorMed's customers never use the miniapp — their coaches do everything for them. A coach in a channel with managed customers switched on (SuperiorMed only) sees **+ 新建托管客户** in the coach panel: name, date of birth and sex (required — BioAge needs a real age), contact phone and the customer's id in SuperiorMed's own system. The customer appears in the client list with a **托管** badge.
  - The coach runs the Kino scan and Dots formulation for them from the existing toolbox, and **asks Viva about them** in the client's chat tab: Viva answers the coach, about the customer, in the third person, using the customer's own data. Regular clients' chat tab is unchanged (a coach note), and a coach can never write as a client.
  - Managed accounts can't log in (no WeChat link, no login phone), aren't charged for Viva (SuperiorMed pays), get no check-ins or program nudges, and are never auto-merged with another account.
  - The channel admin **releases** a customer from the web admin panel's user drawer: they become a regular user, and their contact phone becomes their login phone (proved by their first OTP) unless someone else already uses it. Their coach and all data stay.
  - **SuperiorMed pays with prepaid Dots codes.** SuperiorMed buys codes on the storefront like any store; the coach spends one per customer in the client's chat tab (**兑换码下单** — code plus shipping, pre-filled with the customer's name, phone and the last address used), and activates the box when it arrives (**扫码激活盒子**). The coach becomes the order's buyer on GCN; the formula is the customer's. The coach must have opened the store once from the app, which links their store account. Needs the matching GCN `mall` change (deployed to GCN dev).
  - **Everything in a managed customer's thread speaks about them, to the coach** — the scan result line (both the simulator and the real reader), the formulation proposal and its no-scan refusal, toolbox health advice and image analysis, not just the coach's own questions. Regular users see no change. `docs/architecture/managed-customers.md`. Verified on dev through the API and by driving the coach panel in WeChat DevTools. Migration `migration_managed_customers.sql` (dev applied).

- **API auth, phase 3: every user signs requests with their own session; the miniapp's admin pages are gone** · 2026-09-23
  - Why: the miniapp and web user-app sent one shared token that the server treated as superadmin, and named the user with an `openid` parameter anyone could change — so any user could read or write anyone's data. Needed before the Superiormed channel's coaches manage customers who never log in. Dev only so far.
  - **Per-user sessions.** Every login response now carries `session_token` (signed, 30 days, renewed daily by the clients). The server reads the caller's roles and coach rows fresh on every request.
  - **What a session may do** (`lib/userAccess.js`): only the ~145 routes the two clients actually call, and only about users it may act for — the caller, the caller's coaching clients (`users.coach_id`), or users of the same channel for a channel admin. Records named only by id (coach notes, invitations, orders, plans) are checked through their owner. `PUT /users` from a session can no longer write roles, channel, coach, phone or email; `GET /oss/presign` signs only `academy/` files for a session; the Kino simulator routes need an admin role; an account can only be deleted within an hour of signing up.
  - **Found and fixed on the way: two no-OTP account takeovers.** `/wx-login` accepted a raw `phone` in the body and re-linked the account holding that number to the caller's WeChat — any new WeChat user could take over anyone's account and lock them out. `/wx-app-login` matched accounts on a raw phone the same way. Now only WeChat-verified phones count, and the re-link needs a verified phone.
  - **Miniapp** (VERSION `0923-3`): `utils/session.js` stores the session and renews it; an install signed in before this build swaps the old token for its own session on first launch, with no re-login. A 401 returns the user to login. New versions apply as soon as WeChat has them (`wx.getUpdateManager`). QR-login confirm now sends the session. **`pages/admin` and `pages/superadmin` are removed** — the menu opens the web admin panel for admins.
  - **Web user-app**: signs requests with the session; the shared token is no longer in its bundle. Users signed in on the web before this change sign in once more.
  - **Grace window**: the old token stays accepted, and every use is logged (`legacy_app_bearer`), until `LEGACY_APP_BEARER=reject` is set on prod — then rotate it. Runbook: `docs/architecture/api-auth.md`. Verified on dev by curl and by driving the miniapp in WeChat DevTools as a coach and as a client: every page's requests succeed, and requests about someone else are refused. Tests: `tests/worker-auth-gate.test.js`.

- **API auth, phase 1: no more credential-free account takeover, admin sessions no longer signed with the public app token** · 2026-09-23
  - Why now: the Superiormed channel's coaches will manage customers who never log in, which is only private if the server enforces who may read whom. The 2026-09-04 audit (`TODO.md` "Security") found that impossible today; this is its server-only first phase, which needs no miniapp release. Dev only so far.
  - **Exact public paths instead of prefixes.** The gate exempted everything under `/qr-login/`, `/phone-otp/` and `/email-otp/`, so `bind`, `remove`, `set-primary`, `list` and `accept-unverified` — each taking a `user_id` from the body — worked with no credential at all: OTP your own number, bind it to someone else's account, log in as them. Only `send`/`verify`, `/admin/login`, `/qr-login/init|status|confirm` stay public (`PUBLIC_PATHS` in `worker/index.js`). `confirm` stays public because the released miniapp's `pages/qrlogin` sends no Authorization header.
  - **Fails closed.** `adminCtx` started as superadmin and the whole gate was skipped when `API_BEARER_TOKEN` was unset; it now starts `anonymous` and a missing credential is 401.
  - **`TOKEN_SIGNING_SECRET` signs nano's own sessions.** Channel-admin `ch.` tokens were HMAC'd with `API_BEARER_TOKEN`, which ships in the miniapp — anyone could forge a channel admin with any permissions. New secret, separate `TOKEN_SIGNING_SECRET_PROD` for prod. Existing channel-admin sessions (24 h) stop working once; they log in again.
  - **`/admin/login` returns a signed `sa.` superadmin session** instead of the raw bearer; the kino function accepts it (Hardware tab). The web admin panel no longer falls back to `VITE_API_TOKEN`, which had inlined the app bearer into its publicly served bundle.
  - **QR-login status** returns the user in the phone/email-OTP login shape (`{user, channel, coach}`, named columns) instead of `SELECT u.*`, and a confirmed session is handed out once.
  - **Super OTP** reads its code from the `SUPER_OTP_CODE` secret instead of a source literal (the old code is public in git history); still enabled in prod by decision.
  - **`POST /phone-otp/send` is rate-limited**: 60 s and 5/hour per phone, 30/hour per client IP (`migration_phone_otp_send_log.sql`). It had no limit and costs money per SMS.
  - Prod rollout needs `TOKEN_SIGNING_SECRET_PROD` and `SUPER_OTP_CODE` in `.env` (added), `migrate:prod`, then worker, kino and admin-panel prod deploys. Tests: `tests/worker-auth-gate.test.js`.

- **Media (digital assets) writes now check the admin's permission** · 2026-09-23
  - `POST /digital-assets`, `PUT/DELETE /digital-assets/:id` and `GET /digital-assets/presign` ran for any channel-admin token, so a sub-admin account without the "Media" permission (hidden tab) could still upload, edit or delete their channel's Box files by calling the API directly. They now require `digital-assets:write` (presign, create, edit) or `digital-assets:delete`, like the other admin routes. Superadmins, the miniapp bearer and channel admins with the default full permissions are unaffected; channel scoping in `handlers/digital-assets.js` is unchanged. `GET /digital-assets` stays open — the miniapp's Box tab reads it.

- **Viva on dev, tested with real user questions: sleep-data bugs, invented app screens, symptom answers** · 2026-09-23
  - How found: 17 real questions from dev `chat_messages` (continuations, symptoms, plans, dots, app usage, challenges) sent through the miniapp chat tab with the WeChat DevTools automator, as `37c8774e` (V8 band, 63 Kino scans), with the worker logs read per turn. Routing by the UNDERSTAND step (§47) was right on every turn — both continuations resolved, 「也给我订制一周的营养餐」 did not start a formulation — so every fix below is in data or templates, not routing.
  - **V8 nights read as "no deep sleep at all".** The miniapp summed each session's `null` stages into `0`s, so the prompt said 「深睡 0min · REM 0min」「深睡+REM 占 0%」 and Viva told the user they had probably lain awake all night. Now `sync.js` writes `null` when no session reported a split; `fetchWearableDaily` treats an all-zero split as unknown (repairs stored rows) and renders 「无睡眠分期数据」; `restorative_pct`, `wearableAnalysis`'s restorative figure and `health_twin.avg_deep_sleep_pct` skip those nights.
  - **Sleep onset from a min() over minute-of-day.** A 06:28 top-up after a 21:25 night became the onset (「入睡 06:28，波动达256分钟」), and wake = onset + total minutes. Onset and wake now come from the night's longest session, server-side (`mainSleepSession`, repairs stored rows) and in `sync.js`.
  - **Invented app screens.** 「我的体检报告要传到哪里」 got a 「健康档案」 section and a 「联系护理师」 button; an exercise plan promised the app would draw 「HRV趋势图与呼吸同步率」. New `prompts/chat/appGuideBlock.js` — the miniapp's real tabs, toolbox, upload path, ring binding and menu, labels copied from its i18n — in the casual and lifestyle templates of both personas, for `client === 'miniapp'` (the web user-app sends it too and mirrors the layout); any other client gets only the rule. `llmContext.client` carries it.
  - **Biomarker status left to the model.** One reply called hsCRP 1.16 normal in prose, 偏高 in its own metric card and 升高 later; JUDGE caught four wrong statuses in the first draft and burned two REVISE rounds. Another wrote 「GA…正常范围（<15.5%）」. `prompts/chat/biomarkerStatusLine.js` states `lib/biomarkerStatus`'s verdict and the normal cut-off for each marker in the biomarker and lifestyle templates; the twin line's `avg_deep_sleep_pct` is relabelled 深睡+REM (it always was deep+REM).
  - **Symptoms answered with invented physiology.** 「小腿浮肿是什么原因」 got an HRV→capillary-pressure→edema chain, kidney and heart causes "ruled out" from Cystatin C and the band, today's partial 53 steps read as a collapse, and no warning signs. The biomarker templates' "cross-analyse (睡眠不足→IL-6升高→…)" rule asked for exactly such chains; it now says side by side, never causal, and a new symptom rule asks for common causes, red flags that need a doctor, and what the data cannot answer.
  - JUDGE is told `wearable_daily`/`wearable_insights` are ground truth (it rejected 「HRV连续5天低于个人正常区间」, a computed `low_streak`, for having only three daily rows in view) and that a null stage is "not recorded". The grocery block forbids vouching for anything the tool did not return (「所有食材均已在盒马目录中确认在售」 with five products checked). The casual templates answer a challenge (「你不是智能的吗」) plainly instead of 「我确实不是'智能'的…我不生成答案」.
  - Re-tested on dev after deploying (same day, three rounds). Fixed live: the upload path, the six-marker statuses (and 187 s → 54 s), sleep onsets, the swelling answer (now general causes + red flags, 176 s → 72 s). Three more causes surfaced and are fixed:
    - **The stored twin was stale.** Chat reads `health_twin.wearable_insights`/`avg_deep_sleep_pct`, rewritten only at the next ring sync, so the fixed worker still said 「深睡+REM 0分钟」. `temp/recompute-health-twin-sleep.js --env dev|prod` recomputes the twin through the production `updateHealthTwin`; run on dev (20 users). **Run it on prod after deploying the worker there.**
    - **"No stage data" read as "no deep sleep".** The per-day line now says 「设备未测量睡眠分期」 plus a usage rule that it is not the absence of deep sleep; `get_wearable_daily` replaces the four `null` stage fields with `sleep_stages: 设备未测量睡眠分期（不是没有深睡）`.
    - **JUDGE hints wrote new claims, and a late off_topic swapped the answer.** Hints dictated 「…功能性驱动因素」「没有…器质性病变」 and REVISE pasted them (plus 「经批准的知识库条目」). JUDGE is now told a hint only removes or corrects, and REVISE takes the correction only. Separately, a RE-JUDGE flagged a correct exercise plan off_topic and the new-reply rewrite answered the Kino question; `runAgenticTurn` now drops an off_topic raised on a re-judge when the first JUDGE found the draft on topic (`agentic_rejudge_off_topic_dropped`). The takeaway card may not repeat an earlier turn's.
  - Not fixed: latency — eight of the fourteen agentic turns took 176–242 s end to end, and four of the first nine (the ones with logs read) spent the whole REVISE budget; the status line and the sleep fixes remove the two rejection causes seen, which should shorten them, but that is unmeasured until redeployed. A repeated 「山药梨汤…支持NAD+代谢稳态」 takeaway (a mechanism JUDGE passed twice) is untouched.
  - Files: `lib/wearableDaily.js`, `lib/wearableAnalysis.js`, `lib/healthTwinUpdater.js`, `handlers/chat.js`, `prompts/chat/{appGuideBlock,biomarkerStatusLine,groceryBlock}.js`, `prompts/{viva,nano}/chat/{casual,lifestyle,biomarker}.js`, `prompts/viva/judgeTemplate.js`, miniapp `utils/wearable/sync.js`, `utils/config.js` (VERSION `0923-2`), `lib/agenticChat.js`, `lib/agenticTools.js`, `prompts/chat/outputFormat.js`, `temp/recompute-health-twin-sleep.js`, tests `wearable-daily`, `chat-app-guide`, `chat-biomarker-status-line`, `agentic-late-off-topic`.

- **Chat tab: a reply that repeats the previous one word for word is no longer swallowed** · 2026-09-23
  - Why: on dev, 「你是什么大模型？」 then 「你背后是什么模型？」 got the identical reply twice. The second was saved and its notification sent, but `_poll`'s cross-channel de-dup keys on normalised text alone, so it treated the new reply as the first one arriving on its second channel and dropped it — the typing dots vanished and no bubble appeared. Any repeated reply (a second 「好的，已记录：…」, a repeated greeting) hit the same path.
  - Now: `handlePostChat` returns `user_message_id` (the persisted user row) on the sync and `processing` paths. The miniapp tags each rendered AI key with the turn it was rendered in (`_turnSeq`) and, once a turn is scoped (`_scopeAiDedupeToTurn`), counts only renders from that turn as duplicates. Catch-up rows at or below the turn's user message are earlier turns' replies and keep the old any-turn rule, so an old reply is not replayed. Without `user_message_id` the behaviour is unchanged.
  - Not changed: the web user-app (`hooks/useNotificationPoll.js`) has the same text-only de-dup and the same bug.
  - Files: `handlers/chat.js`, `pages/main/main.js`, `utils/config.js` (VERSION `0923-1`), `tests/chat-ai-dedupe.test.js`, CLAUDE.md §22.

- **Chat routing: an UNDERSTAND step replaces the message-only intent classifier** · 2026-09-22
  - Why: the intent label picks the *template* — the rules, data, tools and vocabulary the model is handed — so a misread label does not degrade an answer, it writes it in the wrong world (Mary's exercise plan answered with a 盒马 shopping list, same day). The classifier read the message alone, with no history and no user state, at `max_tokens: 60`; five deterministic regex backstops had accumulated in ten days patching individual cases of that blindness.
  - Now: `prompts/chat/understanding.js` + `lib/understanding.js` — one `qwen3.8-flash` call (thinking off) that reads the message with the last 4 turns and a one-line user state (`has_kino`, `has_wearable`, active dots plan, memory facts, health plans), says in Chinese what is being asked, and then names a route. Measured against hand labels (`temp/understanding-harness`, local): **85.8% → 96–99%** on the eval set (three runs of the same prompt at `temperature: 0.1`: 98.7%, 96.8%, 96.1% — the spread is run-to-run noise, so read it as mid-90s vs mid-80s), **88.1% → 94.0–97.0%** on a held-out set labeled before the prompt was frozen, and **50% → 83–92%** on messages that need conversation history. Both of the classifier's held-out `formulate_dots` false positives (「请重试分析」,「只需要6号和9号 别的不需要么？」 — each would start a real formulation in the miniapp) are routed correctly.
  - `CHAT_UNDERSTANDING_MODE`: `off` | `shadow` (default — both routers run, the classifier still decides, disagreements logged as `msg:'route_disagreement'`) | `on`. `s.yaml` sets `on` for dev, `s-prod.yaml` `shadow`: the measured win is entirely in the turns where the two disagree, so prod collects that log before flipping. Every failure shape (unparseable JSON, unknown route, 25s timeout) returns `{ok:false}` and falls through to the classifier.
  - Deliberately **routing only**. Three end-to-end rounds on dev through the real `handlePostChat` (14 pairs, graded blind 3× by a data-aware grader) found that injecting the understanding into GENERATE made replies *worse* on the 11 of 14 pairs where both arms routed identically, while the 3 pairs where routes differed were decisive wins (a bare 「16号是什么」 2.0→5.0 and an oral-symptom question 3.0→5.0 — both `casual_chat` misroutes, Mary's failure class). The only thing that reaches GENERATE is `resolvedRequestLine()`: one line, and only when the message is a continuation.
  - When the understanding routes, the five regex backstops keep only their demotions **away from `formulate_dots`** (that route starts a real formulation; a message matching those regexes is never a request to formulate). The promotions out of `casual_chat`/`nutrition_question` are gated off — the accuracy above was measured without them.
  - Model (2026-09-23): the numbers above are qwen3-max; re-scored on the shipped prompt, `qwen3.8-flash` with `enable_thinking: false` ties it (97.4–98.1% eval, 95.5–97.0% held out, two runs), so it ships in its place on cost. `qwen-plus-latest` was slower (~10 s median), returned 3 unusable outputs and sent 「宏基因检测」 to `formulate_dots` in both runs. Thinking stays off (`UNDERSTANDING_THINKING`, default `off`): with it on, calls took 15–45 s and all hit the 25 s timeout.
  - Cost: ~6 s / ~3.3k tokens per turn against 0.7 s / 1.3k, plus two small indexed queries for history and state.
  - Files: `prompts/chat/understanding.js`, `lib/understanding.js`, `handlers/chat.js`, `s.yaml`, `s-prod.yaml`, `tests/chat-understanding.test.js`, `docs/ai-persona/11-intent-understanding.md`, CLAUDE.md §47.

- **Chat: an exercise-plan request no longer comes back as a grocery list — `lifestyle_question` intent, food-gated grocery forcing, JUDGE relevance check** · 2026-09-22
  - Incident (prod, Mary, 16:00 SH): 「根据我的情况定制运动方案」 was answered with a 盒马 ingredient-substitution table for a meal plan from 09-15. Every stage did what it was built to do: the classifier has no lifestyle intent so it chose `nutrition_question`; PLAN (which sees only the message and the intent label) requested `get_grocery_products`; `buildForcedToolQueue` pinned `tool_choice` to it, so GENERATE mined the week-old meal plan out of history for keywords, got an empty result, and answered the tool result instead of the user; JUDGE grades facts, never relevance, so it only trimmed a JD/Tmall availability claim and PASSed the rewrite.
  - Intent: `lifestyle_question` (exercise / training / sleep routine / daily schedule / breathing or stress practice) in `intentClassifier.js`, with `nutrition_question` told to exclude it; new templates `prompts/{viva,nano}/chat/lifestyle.js` built from the shared blocks but carrying **no** grocery, store-product or formulation-package block; registered in both prompt maps (`handlers/chat.js`, `index.js`) and in `HIGH_RISK_INTENTS`. A deterministic backstop (`prompts/chat/lifestyleRequest.js`, `messageAsksForLifestylePlan`) demotes `nutrition_question`/`casual_chat`/`formulate_dots` to it, in front of the `launch_tool` branch — same pattern as the meal-plan guard.
  - PLAN: `planTemplate.js` now lists `get_grocery_products` only when the user's own message asks what to eat. `buildForcedToolQueue` (`lib/agenticChat.js`) forces a PLAN-requested grocery tool only when `messageMentionsFood(message)` (`mealPlanRequest.js`, wide food vocabulary) — otherwise it stays available under `tool_choice:'auto'`. Deterministic triggers and every other PLAN tool are unchanged.
  - JUDGE: `judgeTemplate.js` gains one deliberately narrow `off_topic` category (material): the user asked for one concrete thing and the draft's substance is about something else entirely. REVISE detects it and asks for a **new** answer to the quoted user message instead of "rewrite the SAME reply" — a clause-level patch cannot fix a reply about the wrong subject.
  - Tests: `tests/chat-lifestyle-intent.test.js`. Dev: verified live against the reproduced prod shape (meal plan in history, 盒马 active).
...
- **`/coach-users` and `/coach-list`'s client count are channel-scoped** (`worker/handlers/coaches.js`, `worker/index.js`, `tests/coach-channel-scope.test.js`)
  ...
  - `/coach-list`'s `user_count` counted assigned clients with **no** channel condition (`channelFilter` applies to `u`, the coach's own row; `LEFT JOIN users assigned` had none). Scoping only the roster would have left a channel admin reading "20 clients" above a list of 7, so both moved together. The new condition sits in the JOIN's `ON` clause, never the `WHERE` — this is a LEFT JOIN, and moving it out would drop every coach with no clients in the caller's channel off the list entirely instead of showing them with a count of 0.
  - **No caller that exists today loses anything.** The coach miniapp and the WeChat bridge both authenticate with `API_BEARER_TOKEN`, whose branch leaves `adminCtx.channelId` null — it is assigned only in the `ch.` branch — so a coach whose clients span channels keeps seeing all of them. Only `ch.` channel-admin tokens narrow, which is the case this closes.
  - `/channel-coaches/<id>` had the identical stale count in **both** of its branches and is scoped the same way. Not a copy of the other fix, though: its subtree branch selects coaches with `JOIN subtree`, so its count is scoped to `assigned.channel_id IN (SELECT id FROM subtree)` — using `= $1` there would have swapped the old error for its opposite, counting only the root channel while listing coaches across every descendant.
  - Found via the autobot bridge: its channel-filtered scan had been reporting 7 where the truth is 20, which is what exposed the cross-channel assignment in the first place.
  - **Still open, deliberately:** `/channel-coaches/<id>` takes its channel from the path and receives no `adminCtx`, so a `ch.` channel admin can request any channel's coach list by id — the same class of hole `/coach-users` had. Left alone because unlike `/coach-users` this endpoint exists to answer "give me channel N's coaches", so constraining it is a product call about who is meant to call it.

- **Coach filters on `GET /users`** (`worker/handlers/users.js`, `tests/users-coach-filter.test.js`)
  - A `/users` row carried `coach_id` (a `coaches.id`) and `coach_name` but not the coach's own `users.user_id`, and `GET /coaches` is not a route — the path is POST-only, so a GET returns `Unknown GET route`, which reads like an empty coach list rather than a missing route. A caller that knew *who* a coach was but not their `coaches.id` therefore had to page a whole channel and join on `coach_name`, which is not unique: 749 rows fetched to find 7.
  - `filter_coach_id` and `filter_coach_user_id` narrow the list; `coach_user_id` joins the SELECT, mirroring what `WEBVIEW_USER_SELECT` in `handlers/login.js` already exposes. Both conditions read through the `p` alias — `LEFT JOIN coaches p ON u.coach_id = p.id` is already there for `coach_name` — so neither needs a subquery or a new join, and matching on `p.user_id` excludes unassigned rows for free.
  - **The coach condition is pushed after the channel condition, never instead of it.** A channel-scoped admin passing another channel's coach must still see only their own channel; that is the one invariant the test really exists for. The four dashboard stat aggregates are deliberately left channel-level — coach-scoping them would quietly change what the admin panel's headline numbers mean — and a non-numeric `filter_coach_id` is dropped rather than handed to Postgres.
  - Requested by the autobot WeChat bridge. Note that the same job is already doable with no code at all: `GET /coach-list` returns every coach's `id` **and** `user_id`, so `/coach-list` → `/coach-users/:id` maps a coach and fetches their roster in two small calls. These filters are the answer when the caller wants the standard user shape with search, sort and pagination.
...
- **A formula already ordered stops offering to be ordered again** (`worker/handlers/dots.js`, `tests/formulation-orders.test.js`)
  ...
  - **Scoped to orders that have not got their recipe yet** (`pending_payment`, `paid`, `awaiting_formulation`, `awaiting_ag`), deliberately not to every in-flight order. A package already at `compounding` or `shipped` has its formula attached, so a proposal made afterwards is a genuine next-cycle formula and has to stay orderable. `cancelled` / `refunded` release it again, and only `'proposed'` rows are ever suppressed — an `active` plan is the one row this tab has always had to show.
  - Suppression keys on the order's **stage**, not on `intended_nano_plan_id`. That column names what the buyer was looking at and is advisory (it can name a superseded formula), but there is at most one un-submitted proposal per user, so "an order is waiting for a formula" already identifies it without trusting a link that may point at something nothing will compound. Verified against the live dev rows: the order's `intended_nano_plan_id` did match the suppressed plan.
  - 4 new DB-free tests, including the two that stop this being over-applied: a next-cycle proposal alongside a compounding order, and an active plan alongside a pending one.

- **Plans ▸ Dots leads with the formulate tool when you have no formula, and stops offering the Neo dispenser** (`nano-miniapp/pages/main/main.{js,wxml}`, `nano-miniapp/utils/config.js`, `tests/dots-subtab-order-card.test.js`)
  - The order card sent every user to the GCN store with no plan id. Buying first is a real flow — the order parks at `awaiting_formulation` and nano messages the buyer to run 营养定制 — but for someone who has never formulated it is the long way round: they pay, then get handed back the same job one screen further away. The card now runs that tool instead, landing on the chat tab so the messages it posts are on screen as they arrive.
  - **And only that.** Once a formula exists the package row above already carries 按此配方下单, which names the plan so GCN prices that exact recipe — a second card beside it opening the store with no plan id is a strictly worse route to the same product. So the store branch, its handler and its three strings are gone rather than kept as a fallback.
  - The **Neo dispenser** bind card is off behind a `neoAvailable` flag (false): the hardware is not shipping, so it offered something nobody could act on. Gated rather than deleted, since nothing about the dispenser itself changed and this is its only entry point. The order card's copy no longer explains itself as the fallback for not having one.
  - 5 source-level tests pin both: that the Neo gate is false, that exactly one order card exists and it is never the store one, that `hasProposedFormula` is cleared when the load fails rather than hiding the only route to the tool, and that both languages resolve every string — a mistyped `t.` key renders as empty text, so a blank button ships looking fine.

- **Plans ▸ Dots now shows where every dots package actually is, and lets you say which one a formula fills** (`worker/handlers/dots.js`, `worker/lib/gcnClient.js`, `worker/index.js`, `nano-miniapp/pages/main/main.{js,wxml,wxss}`, GCN `mall/{index,formulation-orders}.js`, `tests/formulation-orders.test.js`)
  - A custom-dots journey spans two apps and the miniapp could see almost none of it. Nano's entire record that a GCN order existed was `users.custom_formulation_purchased_at` — one timestamp, no order id, no status — so a buyer had to open the store webview to learn whether their capsules were being compounded or already in the post. The Dots subtab rendered only the `status='active'` plan, which by design excludes both a `'proposed'` formula from the chat tool and an `'approved'` one already being made.
  - The subtab now lists **one row per package**, merged server-side from the GCN order (paid → compounding → shipped, with courier and tracking number) and nano's own plan row (proposed → approved → active, with day *N* of 28). Either half alone still produces a usable row: a formula nobody has ordered yet, or an order whose recipe does not exist.
  - **Nothing is mirrored.** The order half is read from GCN at request time and never copied into a nano table — a cached status has nothing to reconcile itself against when an order is refunded or filled elsewhere, the same reasoning the codebase already applies to not stamping a flag for "a package is waiting". No migration on either side; every field already existed.
  - **The stage a user sees is not either system's status column.** Two stages have no GCN order behind them at all, and `awaiting_formulation` splits by which package was bought — a fast-track one is waiting on the buyer, a premium one is Viva AG's to fulfil and needs nothing from them. Offering a submit CTA on the latter routes someone somewhere that cannot fulfil their order.
  - **Selection, because "oldest wins" was silently deciding.** Both halves of the fast-track handoff were hardcoded `ORDER BY created_at ASC LIMIT 1`. GCN's `formulation_already_in_progress` guard only runs at order *creation*, so two checkouts started while both were `pending_payment` can both be confirmed — leaving two waiting packages and no way to say which one a formula was for. `_attachRecipeToAwaitingOrder` now takes an optional `order_id`; ownership is still enforced by the `nano_user_id` predicate rather than by the caller having supplied a plausible id.
  - The chat card resolves the choice at **tap** time rather than from the card, which is also more correct: the formulation turn is async, so a card can be minutes old and an order refunded or a second one confirmed since it rendered. With one package waiting nothing changes; with several the user picks from an action sheet. In the subtab, tapping a package row *is* the choice. Both surfaces share one submit path so they cannot disagree about what a reason code means.
  - A waiting package is **offered** the current proposal (`submit_plan_id`) rather than matched to it — nothing is bound until the user submits — and the standalone proposal row is suppressed once offered, since the same formula listed twice reads as two formulas. The buy-another card hides while any package is in flight, because GCN would refuse that purchase anyway.
  - 18 DB-free tests pin the merge: all three order↔plan links, that `intended_nano_plan_id` is advisory and binds nothing, that one plan is claimed by one order, that an active plan outranks a stale `shipped` order, every status mapping, the clamped day counter, and that `_awaitingOrders` re-sorts oldest-first to agree with what GCN will actually attach to.

- **"我要定制营养素" now launches the 营养定制 tool instead of being answered with an essay** (`worker/prompts/chat/intentClassifier.js`, `worker/handlers/chat.js`, `nano-miniapp/utils/tool-actions.js`, `nano-miniapp/pages/main/main.js`, `tests/chat-launch-formulate-tool.test.js`)
  - Asking for a custom formulation in the chat tab is a request to **act**, not a question. It was being routed to `nutrition_question`, so the user waited out a 60-180s agentic turn to be handed prose describing a formulation that had not been made — the tool sitting in the toolbox is the thing that actually makes one.
  - The classifier gained a `formulate_dots` intent, explicitly scoped to starting/re-doing/ordering *their own* formula. Asking what a dot does, what is in their current plan, or what custom nutrition even *is* ("定制营养素是什么") stays `nutrition_question`.
  - `handlePostChat` short-circuits on it **before the data fetch and before any generation**, returning `{launch_tool:'formula_dots'}`, and the miniapp runs the same tool the toolbox button runs. Cheaper than the turn it replaces: one classifier call, no agentic loop.
  - **Only the miniapp is told to launch it**, because it is the one client wired to run a tool off a chat reply. The coach app, the web user-app and sandbox ("login as") sessions degrade to a normal nutrition answer rather than a silently dropped turn — and sandbox must never write a real formulation against the impersonated account.
  - The user's own message is persisted by the branch that returns early, and the tool skips its canned "请帮我配制我的 DOTS 方案" trigger line (`skipUserMsg`), so the exchange doesn't read as the user asking twice.

- **Buying a package now attaches the formulation you were looking at, and 查看配方 stops claiming to be your activation codes** (`worker/handlers/users.js`, GCN `migration_0086` + `mall/{index,formulation-orders}.js` + `dashboard.html`)
  - Reported live: 轻逍 ordered 原粒 · 定制营养素 · 28天 from the miniapp, tapped 查看配方, and got a modal titled **我的激活码** containing nothing but `配方主打方向: —`. Two causes. The modal is shared by three item kinds and its heading was hardcoded to the activation-code one, so a buyer who tapped 查看配方 was told they were looking at activation codes. And a buy-first package is paid *before* anything is formulated, so the recipe block had nothing to render and printed an empty shell.
  - The heading is now taken from what is actually below it (我的配方 when a formulation line is present), the focus line is dropped when there is no focus rather than printing an em dash, and a recipe-less line says whose move it is. That answer is per-product, so `is_ag_formulation_bundle` / `requires_expert_review` are carried onto the order item: at `awaiting_formulation` a fast-track package waits on the buyer running 营养定制, while a premium one waits on Viva AG and needs nothing from them. `order.status` alone cannot tell those apart, and getting it backwards would either strand the order or send someone somewhere that cannot fulfil it.
  - **The deeper cause: the buyer tapped a button that said 按此方案定制下单 and got an order referencing no formulation at all.** GCN's `dashboard.html` decides where the miniapp's `buy_custom_formulation` intent lands, so the production miniapp reached the new tier picker the moment the web function deployed — while its own card copy still promised to order *that plan*. The plan id is now carried into the order (`order_item_custom_formulations.intended_nano_plan_id`) and payment confirmation asks nano to attach it.
  - **A separate column, deliberately.** `nano_nutrition_plan_id` is the audit record of what will be compounded and is what `formulation_reviews` resolves against; `intended_nano_plan_id` is advisory and may never be compounded. Collapsing them would make an unpaid, unvalidated intention indistinguishable from a committed formula.
  - Attaching is **only ever an attempt**. `handlePostFormulationSubmit` re-checks ownership, that the plan is still `'proposed'` (a newer formulation supersedes it), that an order really is waiting, that it fits the purchased weekly width, and that the expansion still passes the validator. Every refusal falls through to the existing nudge, so nothing here can leave a buyer worse off than before — except the one refusal worth explaining, an over-tier plan, which now says so rather than sending them to regenerate the same too-wide formula.
  - Verified end-to-end on dev through the deployed functions: proposal → package order recording it → seller confirms payment → nano auto-submits → order `compounding` with 56 capsules attached, the nano plan linked to the GCN order, and the buyer told it is being compounded rather than told to go formulate. On prod, the deployed order-detail endpoint returns the two flags for 轻逍's real order, and 9 browser checks cover the waiting-note copy for every product/status combination.

- **The 营养定制 tool now sells 原粒 · 定制营养素 · 28天, and the package's tier caps the formula** (`worker/handlers/dots.js`, `worker/handlers/chat.js`, `worker/handlers/users.js`, `worker/lib/gcnClient.js`, `worker/prompts/{nano,viva}/systemFormulaGenerate.js`, `nano-miniapp/pages/main/main.js`, GCN `migration_0085` + `mall/{index,formulation-orders}.js` + `dashboard.html`, `tests/formulation-package-tier.test.js`)
  - The formula card's order CTA opened the **formulate→buy** product, priced per dot from the recipe the user was looking at. It now opens the **buy-first** package: pick one of three tiers (6 / 8 / 10 种原粒), pay a flat price, and the order parks at `awaiting_formulation` with no recipe — the ordering the product was actually designed around.
  - **The tier caps one WEEK, not the cycle**, so a formulation may rotate: six dots this week, a partly different six next week, never more than six at once. That is why a recipe now carries an optional per-dot `weeks` map — a key naming no weeks is in every week, so a proposal stored before this, a stale cached completion and the deterministic fallback all still expand to four identical weeks with no migration. `_planExpansionContext` budget-fits once per week, because a week running five of a formula's twelve dots has capsule room the others don't.
  - **The model owns the rotation, the server owns the width.** Whether a dot can be paused for a week is a clinical judgement — continuous sleep support and a seasonal accent are not interchangeable — so the prompt asks for the `weeks` field and says continuous dots stay in all four. `_capDistinctDots` then only enforces the limit, and enforces it *per week*: a dot cut from an over-full week 3 keeps weeks 1-2, and only a dot left with no weeks disappears. Applied **once**, before the recipe is stored, so the card, the box scan and the fast-track submission all expand one recipe and cannot disagree.
  - `DOT-N7` is outside the count: the system's reset dot, dosed alone on 2 of the 28 days in every plan regardless of tier, so counting it would silently cost a 6种 buyer one of the six dots they paid for. `_countDistinctDots` measures the **widest week**, never the cycle's distinct total. Dropping reuses the daily-budget rule's own ranking (`_emphasisPosition`, now shared) — a dot dropped for capsule space and a dot dropped for the tier are the same judgement about the same recipe.
  - `handlePostFormulationSubmit` refuses (never trims) a plan that exceeds the purchased tier: a plan proposed *before* the package was bought was capped by nothing, and re-running 营养定制 produces a better formula than that one minus a dot.
  - **Known limit left open:** `_getCommittedPlanDay0Breakdown` reads day 0, which under rotation is week 1 rather than the cycle. Both its callers want the cycle-wide union — GCN's per-dot checkout snapshot (changing it changes what a customer is charged) and the printed box label (which would under-list a box holding all four weeks). Documented rather than quietly redefined.
  - **A paid package now says so in chat.** An order waiting at `awaiting_formulation` needs something only this app can produce, and nothing in the store said so — GCN's payment confirmation now carries the package, and nano delivers a `formulation_order_paid` message inviting the user to run 营养定制. An expert-review (Viva AG) package sends none: the user has nothing to do there.
  - Nothing about the tiers is compiled into the client. The picker reads `GET /api/mall/aeviva/formulation-packages`, so a tier added, repriced or retired in the admin panel takes effect with no deploy — and if that returns nothing, the CTA falls back to the per-dot product rather than dead-ending.
  - Verified against dev on both sides: the tier reaches nano through the real handler on a real order row, the client mapping degrades to "uncapped" on a missing/garbage/zero tier, the checkout renders and orders the selected tier through the buyer's own bound store (18 browser checks against the shipped functions), the paid-package nudge lands on both delivery channels under the right persona, and a rotating formula survives the full DB round trip — proposal → `proposed_recipe` → box scan → 56 schedule rows — with the chat card agreeing with the written schedule day for day.
...
- **A coach session whose `coach` object was lost now repairs itself instead of dead-ending** (`worker/handlers/login.js`, `worker/index.js`, `nano-miniapp/pages/coach/coach.js`, `tests/coach-panel-empty-states.test.js`)
  - Found by the diagnostic line added in the same session — a 体验版 build reported `nano.gcn.net · coach=null · v0830-1` on screen, which named the cause in one look after it had previously taken repeated rounds of live prod debugging to not find.
  - **`globalData.coach` was captured at login and never refreshed again.** `nano_user` is rewritten from the server in several places (`main.js` 1579/3105/4411), but `nano_coach` is only ever written at login and on sandbox enter/exit. So a session that once stored a null keeps it across every relaunch, permanently — while `roles` *do* get refreshed. That is how a user the app labels 教练 ends up with no coach id at all, and a client list that can never load.
  - New `GET /api/my-coach?user_id=` re-resolves it through the same `resolveCoachSession` every login path uses. Roles are read from the database rather than taken from the caller, and a non-coach gets `{coach: null}` — a normal answer, not an error.
  - The coach panel now attempts that repair before showing the no-coach state, writes the result to **both** `globalData` and storage so it survives the next launch, and tries once per page instance rather than on every reload.
  - Verified end to end by reproducing the exact broken state in the simulator against prod — a real 黄毅 session with `coach` forced to null — then opening the panel: 37 clients, empty error state, and the repaired coach persisted to storage.

- **The coach panel says why its client list is empty, and refetches when you come back to it** (`nano-miniapp/pages/coach/*`, `admin-panel/src/tabs/CoachTab.jsx`, `tests/coach-panel-empty-states.test.js`)
  - A coach with 37 real clients saw "暂无分配的客户". Diagnosing it took several rounds against live prod, because **three unrelated causes rendered as that one screen**: no coach id in the session (so `/coach-users/:id` was never called — the old code substituted `Promise.resolve({ data: { users: [] } })`), a non-200 response (`_req` resolves on any status and only rejects on transport failure, so `res.data?.users || []` silently became `[]`), and a genuinely empty list. The app discarded the one fact that would have answered it immediately.
  - Each case now has its own message, and a failed load offers a retry. On failure only, a muted line reports **backend host · coach id · build** — the three facts that were the whole investigation. It stays hidden for a coach who simply has no clients.
  - **`onShow` refetches.** The list was previously fetched only in `onLoad`, which fires once per page instance, so a backgrounded panel kept showing whatever it loaded the first time — indefinitely, with nothing indicating it was stale. The first `onShow` is skipped, since `onLoad` has just fetched.
  - A broken coach session no longer empties the 邀请码 tab as collateral: invites key on the user id, not the coach id, so they still load.
  - **Admin panel: the same view no longer shows a stale count beside a fresh list.** A coach row's `用户数` comes from the `/coach-list` fetch at page load and cannot know about assignments made since — it read 0 next to a modal listing 37. Opening a coach's user list now corrects that row's badge from the response it just fetched, which is authoritative for that coach.
  - Tests read the real page source: that the branches stay distinct, that every new string resolves in **both** language blocks (WXML has no compile-time key checking — a missing key renders blank), and that no comment node sits between the `wx:if`/`wx:elif`/`wx:else` siblings, which breaks the page on Android and is invisible in the simulator.

- **Phone-OTP login returns the caller's `coach` object, matching the WeChat login paths** (`worker/handlers/{login,phone-otp}.js`, `tests/coach-session-on-login.test.js`)
  - **Repo/prod reconciliation, not a live bug fix.** Deployed prod already returns `coach` from `/phone-otp/verify` (verified live for two accounts); every committed branch in this repo returns `{ user, channel }` without it. So a deploy from this tree would have *removed* it — the drift is the hazard, and this closes it.
  - It matters because the client does `const coach = data.coach || null` (`_finishLogin`); a null there gives `pages/coach/coach.js` `_coachId = null`, and `_loadAll` then short-circuits to an empty client list **without calling the API**. The panel stays reachable regardless, since its permission check reads `user.roles` — so the failure would present as "no clients" rather than "no session".
  - `resolveCoachSession(userId, roles)` is now the single definition, exported from `login.js` and used by all three login paths. The two WeChat handlers held identical inline copies and the phone path had none, which is how the repo copy came to differ from prod.

- **A coach sharing her personal referral code now gets the signups in her client list** (`worker/handlers/login.js`, `tests/coach-referral-code-assignment.test.js`)
  - Prod incident 2026-08-29 (channel 2, aeviva-china): a coach ran an event and shared her **personal referral code** (`users.referral_code` — what `pages/referral` puts in a share link as `invite=`) rather than her **coach invitation code** (`invitations.code`). Both arrive on `/wx-login` as the same `invite_code` field, but only the invitation branch ever resolved a coach: the referral fallback set `referred_by_user_id` and inherited the channel while leaving `users.coach_id` NULL. The coach client list filters strictly on `WHERE u.coach_id = $1`, so all 36 of that day's signups were invisible to her — the codes look interchangeable to whoever is holding the phone, and nothing on either path said otherwise.
  - **A referral code whose owner is an active coach now assigns that coach**, on all three fallback branches (new signup via `/wx-login` and `/wx-app-login`, plus an existing channel-less user supplying one). An explicit `coach_id` and an already-assigned coach both still win — a referral must never reassign someone's existing coach.
  - **An ordinary user's referral code still assigns nobody.** That is the case the fix must not break, and it is asserted directly: fabricating a coach for every user-to-user referral would be the worse bug.
  - Filters on `coaches.status = 'active'` — a no-op today (every coach row is active) but the coach-invite path's lack of that filter is a deliberate difference: an invite link is an explicit act by that coach, whereas a referral code keeps working passively long after.
  - The 37 already-affected users were assigned to coach 44 on prod by hand — the same single-column write `/api/assign-coach` performs.

### Added

- **Every formulation gets a QR the moment it is generated — one code from formula to box to activation** (`worker/lib/labelCode.js`, `worker/handlers/{dots,boxes}.js`, `nano-miniapp/`, `admin-panel/BoxesTab.jsx`, `migration_nutrition_plans_label_code.sql`; GCN `site/aeviva/formulation-label.html`, `mall/formulation-orders.js`)
  - The box code used to be minted at box-batch time, which is far too late to show anyone: the user had no way to see what they were about to receive, and nothing to track an order by. It is now minted with the formula, as `nutrition_plans.label_code`, and the QR is part of what the chat card delivers.
  - **One code, one space.** A plan's `label_code` becomes its box's `box_code` — `lib/labelCode.js` checks both tables when generating, because a collision across them would mean a scan resolving to someone else's capsules. Multi-box batches give the first box the formulation's own code (the one the user was already shown) and mint fresh ones for the rest; each still claims the same plan.
  - **The QR is a GCN aeviva sector link** (`aeviva.gcn.net/formulation-label.html?c=WVB…`), because that is the sector the product sells in and the page already existed there — it prints, and it draws its own QR. The `WVB` + 12-hex shape is what makes one QR do both jobs: a phone camera opens a readable page, and the Mini Program's scanner reads the code straight out of the same URL via the existing `/WVB[0-9A-Fa-f]{12}/`. A test pins that against the bare code, the old nano box URL, and the new one.
  - **Minting a code does not make anything claimable.** A scan still resolves through `boxes`/`box_batches`, which only exist once a batch is actually compounded — so scanning the QR of a formulation nobody manufactured returns `box_not_found` rather than activating a plan for capsules the user does not have.
  - **`GET /api/formulation-label` is public, and that constrains it.** The code is printed on a physical object, so whoever holds the box can read it — the same reasoning as the older `/api/box/{code}` page it supersedes. It therefore returns **no user identity of any kind**, and truncates the order reference the way GCN's own label already does. A test asserts the user id does not appear in the payload.
  - **GCN proxies it server-side** rather than the page fetching nano directly — nano's routes all need some bearer, and the custom domain in front of nano emits a duplicate CORS header browsers reject. Same shape as `handleNanoFocusTemplates`.
  - The miniapp opens the label in a webview instead of drawing a QR natively, so what the user sees on screen is the exact page that prints on the box, and there is only one renderer to keep correct. `aeviva(-dev).gcn.net` is already a verified WeChat business domain, so no new domain setup was needed.
  - Labels printed before this change keep working: the endpoint falls back to `boxes.box_code`, and the claim regex still reads nano's old box URL. Physical objects already in the world cannot be re-printed.

- **The chat tool knows whether the 28-day package was already bought, and can fulfil it** (`worker/handlers/dots.js`, `worker/lib/gcnClient.js`, `nano-miniapp/pages/main/`, `migration_nutrition_plans_gcn_order.sql`; GCN `mall/formulation-orders.js`, `migration_0081_aeviva_fasttrack_formulation.sql`)
  - A custom-dots order can be placed in either sequence — formulate then buy, or buy then formulate — and the chat card was only ever built for the first. Someone who had already paid was still shown a button offering to sell them the same thing.
  - **The card now has three modes**, carried by a `#order` meta line the server writes: `buy` (offer to order this formulation), `submit` (a paid fast-track package is waiting — offer to confirm THIS formula for compounding), and `ag` (a paid premium package is waiting; Viva AG owns that one, so no CTA at all, just a note saying so).
  - **Pulled from GCN at delivery time, not pushed and cached.** An order can be refunded, cancelled, or fulfilled by an AG run in between, and a stamped flag has nothing to reconcile it against. Resolved when the card is built rather than when the request was made, because the turn is asynchronous and may be minutes old — long enough for a checkout to complete in the gap. **Any non-answer degrades to `buy`**, the safe direction: a buy button someone has already paid past is ignorable, whereas a submit button with no order behind it fails on tap.
  - **Fast track means no human ever sees the formula**, so `validateAgFormulation` — the same validator an externally-authored Viva AG formula must pass — is the only thing between a generated allocation and capsules a person swallows. It runs on submit and **refuses** on any violation rather than repairing anything, the same reject-never-repair rule §36 sets. The expansion is rule-conformant by construction, so a violation means the expansion itself regressed, which is exactly the case worth catching.
  - **No new branch flag on GCN's side.** Three existing SKU columns already express the new product: `is_ag_formulation_bundle` is the *order shape* (flat-priced, no recipe at checkout, parks at `awaiting_formulation` — the "AG" in the name is historical), `requires_expert_review` is the *fulfilment* and is the entire difference between the two packages, and a NULL `viva_subscription_plan_key` means the purchase grants no entitlement. A future third package is a configuration change rather than a code change.
  - **GCN refuses a downgrade rather than honouring it quietly.** `handleFormulationFastTrack` rejects an order whose SKU requires expert review — that flag is the difference between two products at two prices — and nano surfaces the refusal to the user as "that package is formulated by Viva AG". The order-lookup and write-once `recipe_snapshot` half is now shared with `handleFormulationReady` so the two paths cannot drift on what they attach; they differ only in what happens after.
  - Submission is **idempotent** (a second tap returns the order the first attached to, never a second formula for one purchase) and gated behind an explicit confirm dialog that says the formula is fixed at that tap and regenerating is how to change it.
  - The fast-track SKU ships with a **placeholder price** (¥980, below the ¥1980 AG bundle), the same caveat migrations 0061/0064/0078 all carry — real pricing needs the business side before it goes live.

- **Formulate Dots proposes a real 28-day plan again, shown as Day 1–28 rather than dates** (`worker/handlers/{dots,chat,boxes}.js`, `nano-miniapp/utils/markdown.js`, `nano-miniapp/pages/main/`, `web/user-app/`, `migration_nutrition_plans_proposed.sql`)
  - The chat tool had been evaluation-only since the real 28-day formula moved to Viva AG: it rendered an allocation and wrote nothing. That left it with no route to the store — GCN's custom-formulation checkout prices a recipe out of a `nutrition_plans` row, and there was no longer a row to price — so the tool showed a user what their data implied and then offered them no way to get it.
  - **A new `'proposed'` status, deliberately not a reuse of the three that existed.** `'pending'` means an async formulation is mid-flight and may never land; `'approved'` means a nutrition expert signed the recipe off and a batch is being compounded, and conflating the two would let unreviewed model output reach the compounding queue; `'active'` means the user physically has the capsules. A proposal is none of those: it is a real, purchasable recipe for capsules that do not exist yet.
  - **Day numbers are relative because the plan genuinely has no dates yet.** The capsules have to be compounded and shipped, so a proposal carries no schedules at all and the card says `Day 1–9 · 12–28`, never a calendar date. Scanning the delivered box (`_activateProposedPlan`) is the first moment a real day exists — that is when `start_date` becomes today and the 56 capsules are written, mirroring what the Viva AG flow already does at `'approved'`.
  - **It never disturbs the plan the user is on.** Only a previous *proposal* is superseded, so someone mid-cycle on a box they already have keeps taking it while a new proposal sits alongside; a partial unique index enforces one live proposal per user. `handleGetNutritionPlan` filters `status = 'active'`, so a proposal stays invisible to the Dots subtab with no code change — which is also why the card carries the numbers itself instead of a "view plan" button that would have opened the *previous* plan.
  - **Days that expand to the same two capsules are collapsed into one group.** A 28-day plan varies on exactly two days — the `DOT-N7` isolation override — so enumerating 28 near-identical rows in a chat bubble would be noise; grouping keys on the capsule contents themselves, so two days group together exactly when they are genuinely the same dose. Bars normalise against the largest capsule across *all* groups, so a reset day reads as the smaller capsule it is instead of self-normalising to look full.
  - **One expansion rule set, not three.** `_expandPlanDay` is now the single place the N7 isolation override, the pulse window and the 72-per-capsule cap are applied, shared by `_commitNutritionPlan` (writing schedules), `_activateProposedPlan` (the box scan) and `_planDayGroups` (the card). Three consumers that must never disagree about what a user is actually taking; a test asserts the dateless preview matches what the scan will write, day for day.
  - **`_isPulseActiveDate` cannot be evaluated without a date**, since it is anchored to a fixed calendar epoch. With no date, pulse dots are left in every day — exact today (N7 is the only pulse dot and it is routed through isolation instead), but a second pulse dot would make a proposal over-state the days it appears on until the scan anchors the cycle. Recorded at the call site rather than papered over by inventing a start date.
  - **No GCN change was needed after all.** Its checkout reads `snapshot.valid` and the dot breakdown and never inspects plan status, so widening `handleGetFormulationCheckoutSnapshot` to accept a proposal — deriving day 0 from `proposed_recipe`, through the same expansion the scan will use — was sufficient. Day index 0 is never an isolation day, so it yields exactly the steady-state capsules that endpoint already promised.
  - **Card meta lines are `'#'`-prefixed and all optional**, which no dot key can start with, so a card sitting in chat history from before this change still renders as one unlabelled group. Tests cover that, plus the plan id being digits-only before it reaches a `data-` attribute and the day ranges being digits-only before they reach the renderer.
  - **Copy that had drifted, fixed alongside**: the chat tab still said "7天/7-day" (the cycle became 28 days on 2026-08-08 and only the coach page was updated), `formulaComplete` claimed a plan had been generated while the card directly beneath it said "not saved", the coach page pointed reviewers at a Plan page this run does not change, and the web user-app announced "ready!" the instant the POST returned — minutes before the recipe existed. Two dead strings (`formulaViewDots`, and `formulaProcessing` in the chat tab) removed.

- **The health tab and chat tab text can be enlarged, by pinching** (`app.wxss`, `components/user-health/user-health.{wxss,js,wxml}`, `utils/markdown.js`, `pages/main/`, `pages/coach/`, `worker/handlers/users.js`)
  - Older users reported the health tab was too small to read, and it was: its 169 `font-size` rules bottomed out at 14rpx (~7px on a 375pt screen), the most common value was 20rpx (~10px), and `app.wxss` declared no `font-size` at all — so there was no lever to pull. Four levels now scale body text by up to +50%, selected by a two-finger pinch on the tab itself or a stepper in the header menu, and remembered per user.
  - **The gesture is the one that was asked for; the effect is not.** A literal photo-app zoom (`transform: scale`, or `movable-view scale`) was tried on paper and rejected: the tab holds 9 `<canvas>` charts that rasterize blurry when scaled and whose crosshair hit-testing reads a cached `boundingClientRect`, plus 3 `position: fixed` overlays that a transformed ancestor re-parents — and zoomed content overflows horizontally, so it would need a pan layer over the existing scroll-view. Pinching now steps a text size instead: the same familiar gesture, but the text reflows and stays sharp, and every chart and overlay keeps working untouched.
  - **The tokens live in `app.wxss` because custom properties ignore style isolation.** This is not a guess — `user-health.wxss` already resolves 339 `var(--*)` references against tokens defined only on `page {}`. So the 23 size tokens sit next to the colour tokens and the level class goes on the *host page's* root view, the same node that already carries `.theme-light`. The health component needs no root-class change of its own, and extending this to chat/plans/store later is a find-and-replace with no new plumbing.
  - **It is a curve, not a multiplier.** Caption and body text (≤22rpx, where the complaint actually lives) gets the full +15/+30/+50%, while display numerals taper to +15% — a flat 1.5× would take the 88rpx BioAge numeral to 132rpx and burst its chip. The curve is strictly monotonic at every level, so no two originally-distinct sizes collapse into each other and the visual hierarchy survives; a test asserts that property rather than trusting the table.
  - **Level 0 is byte-identical to the sizes it replaced**, and every tokenised declaration keeps its literal as the `var()` fallback. There is no build step and no CSS lint in this project, so an unresolved token would otherwise drop the declaration entirely and fall back to *inherited* size — with the fallback it degrades to today's rendering instead.
  - **14 rules keep their literals on purpose**: glyphs sized inside fixed boxes (a ✕ in a 48×48 button, a numeral in a 36rpx circle, hour labels in a 24-column axis) where scaling clips the box rather than helping anyone read. Two rules that pair scaling text with a hard width were relaxed to `min-width`, and the sleep-week axis padding is now derived from the two label tokens it was hand-aligned to, so the ticks stay aligned at every level.
  - **One step per gesture, latched until touchend** — a continuous pinch cannot run away through all four levels. Both host pages' edge-swipe handlers gained a multi-touch guard, since each read `touches[0]` unconditionally and a pinch starting near the edge could otherwise navigate away mid-gesture; the guard is latched rather than cleared on touchend, because touchend fires once per finger.
  - **The chat tab followed, and the AI's own replies were the hard part.** `main.wxss` is one file for the whole main page, so the conversion is line-range scoped to its chat sections (39 rules, 6 icon glyphs excluded); the other tabs still hold literals deliberately. But the bulk of what a user actually reads in chat is markdown rendered by `mp-html`, and `MD_TAG_STYLE` hardcoded `font-size` on `h1`–`h6`, `code`, `pre` and `table` — applied as **inline styles**, which beat any stylesheet rule, and bound once and never re-parsed. Left alone, body text would have scaled to 40rpx at the largest level while `h1` stayed at 34rpx, rendering **headings smaller than the paragraphs under them**.
  - **Those sizes are now relative (`em`) rather than tokenised**, so they track whatever bubble they land in with no token plumbing inside `mp-html` at all — which matters because its parser rewrites `rpx` to `px` inside inline styles, and because `mp-html`'s own built-in `tagStyle` already uses `em` for `big`/`small`, so this is the library's existing idiom rather than a new trick. The ratios reproduce the previous sizes exactly at the default level (34/28 = 1.21, 32/28 = 1.14, 25/28 = 0.89), and a test asserts that so the two can't drift.
  - **The Viva AG report viewer scales too**, as a consequence rather than a bonus: it renders through the same shared `MD_TAG_STYLE`, so its prose container had to become tokenised or its headings would have gone proportional to a body size that itself never moved. It sits inside the health tab, which already scaled — so this also closes an inconsistency the first pass left behind.

  - **The gesture detector is a shared module** (`utils/pinch.js`). The chat tab is plain page markup while the health tab's gesture lives inside a component, so binding it in both places would otherwise have meant two copies of the same latch and threshold logic drifting apart. Both now drive one stepper, and a test asserts neither re-derives the maths locally. It reports a direction and nothing else — clamping and persistence stay with the caller.

  - **The setting rides in `users.preferences`,** merged with `||` so the dispatcher's `daily_checkin_enabled` opt-out survives the write, and is read back through both user selects so it follows the user to a new device. Storage stays the primary source, and it is hydrated outside `app.js`'s `if (user)` block — unlike theme, an accessibility setting has to survive logout and apply on the login screen.

- **Viva AG can ask the user a clarifying questionnaire mid-job instead of guessing** (`worker/lib/agQuestionnaire.js`, `worker/handlers/{viva_ag,questionnaires}.js`, `worker/lib/twinBundle.js`, `worker/docs/viva-ag-api.md` §7b, `nano-miniapp/components/{viva-ag-panel,user-health}/`, `nano-miniapp/pages/main/`, `migration_viva_ag_questionnaire.sql`)
  - Viva AG was a one-shot pipeline: the external agent read the twin, worked for minutes-to-hours, and delivered. If the twin was missing something it needed — a symptom timeline, a medication no lab panel implies, whether a discharge summary describes a resolved or ongoing condition — it could only guess or fail. That matters most for `dots_formulation`, whose output is now compounded into physical capsules. The agent can now **park** its claimed job (`POST /viva-ag/jobs/questionnaire`), push back up to 12 questions, and resume when the user answers.
  - **Almost nothing was built, because the return path already existed.** Nano already had a server-defined questionnaire system — four tables, five input widgets, a chat-tab renderer, a `questionnaire_ready` client trigger, and a precedent for a runtime-LLM-generated form (`type='dynamic'`). And `twinBundle.js` already merged completed answers into every bundle, so answers reach the agent with **no new plumbing at all**. Only the asking, the parking and the resume trigger are new; the questionnaire is `type='viva_ag'` and `createDynamicQuestionnaire` grew a `type` param.
  - **`awaiting_input` had to join `uniq_viva_ag_jobs_active`.** A parked job still owns the user's one in-flight slot — letting them enqueue a second would leave two jobs competing for the same answers, and the second would resume off the first one's form. `viva_ag_jobs.status` has no CHECK constraint, so that index *is* the migration's real work; the predicate was verified on a live DB, since a `DROP`/`CREATE` pair that silently no-ops would only surface much later.
  - **The lease is released, not held, and the attempt is refunded.** No lease length covers a person answering a form, so the job goes back on the queue and any worker re-claims it fresh with a new fencing token — which is what keeps workers stateless, since the answers travel in the bundle rather than in a worker's memory. And because asking is forward progress rather than a failed delivery, the claim's `attempts` increment is given back: two clarifying rounds would otherwise eat the retry budget a genuinely stuck job still needs. Rounds (2 per job) bound the loop instead.
  - **The validator is a security boundary, not a formatting check.** `questionnaire_questions` is a write path into user data: `save_target` writes `users.<save_field>`, merges `bio_data`, or **inserts a biomarkers row**, and `completion_check` makes a question auto-skip — so an agent could otherwise push a form that instantly self-completes and resumes the job having asked nothing. Those four fields are **never sourced from the payload**; they are hard-written `NULL`/`'{}'`. The same rule strips `config.other_key` and rejects an option keyed `other`, whose free text never reaches `questionnaire_responses` anyway. Prompts render as chat bubbles, so `:::` display-card fences and markdown links get the same neutralisation `result_summary` already receives.
  - **A 7-day deadline stops an ignored form holding the slot forever** — swept by a third statement in `_sweepExpiredLeases`, separate from the two lease statements because those key off `claim_expires_at`, which a parked job has none of. The job then closes as `questionnaire_unanswered` with its own message rather than the generic timeout text, and `cancel` was widened to `awaiting_input` so "I'm not answering this" stays the user's decision.
  - **The user answers in the chat tab**, reusing the one renderer onboarding, coach-assigned forms and Viva's own follow-ups already share; a second renderer inside the AG panel would be a second place to maintain every input type for no user-visible gain. The panel shows a "需要补充信息 · 去回答" card and hands off. `bundle_version` is now **2** — additively adding `job_questionnaires`, the job's own rounds and answers, which `questionnaire_context`'s whole-user merge cannot distinguish.

- **Viva can recommend the Aeviva storefront's non-Dots products, reactively** (`worker/prompts/chat/productRecommendBlock.js`, `worker/lib/gcnClient.js`, `worker/handlers/{chat,dots}.js`, `nano-miniapp/pages/main/`, nano `migration_knowledge_store_products.sql` + GCN `migration_0079_product_ai_profiles.sql`)
  - The `aeviva-china` admin's GCN catalog carries real products beyond Dots — supplements, skincare, wellness devices — and none of it reached the AI. The blocker was not ignorance but instruction: the always-injected `fact-constraint-core` block said every ingredient suggestion must come from the Dots formulary and that no purchase channel may ever be named, so no amount of catalog data in the prompt would have overridden it. That rule is now **narrowed, not removed** — recommend from the Dots formulary *or* a catalog explicitly provided in this prompt, never from training-data memory. With no catalog present the parenthetical collapses it straight back to Dots-only.
  - **GCN products get a reviewed AI profile, deliberately separate from their marketing copy.** `products.description` already holds `zh`/`highlights_zh`/`details_zh`, but that copy exists to convert a shopper already looking at the item; handed to a health LLM it becomes personalized medical advice assembled out of unvetted promotional claims — precisely what the guardrail was written to prevent. `product_ai_profiles` instead carries a factual summary, key actives with doses, cautions, allergens, and `sub_age_targets` using nano's canonical §11 keys. That last field is the one that earns the table: it makes "which products fit this user's elevated dimension" a **code filter**, the same way `dots.sub_age_target` already works.
  - **A supplier writes the copy; only a sector admin may make it recommendable.** `ai_recommendable` is admin-only and cannot be set without a `reviewed_by` — the same gate `knowledge_entries` enforces before an entry goes active, and the same precedent as `skus.grants_partner_type`, the one SKU field that rejects its owning supplier. A supplier's routine copy edit can never silently un-approve a product an admin had cleared.
  - **Reactive only, and structurally so.** The catalog is fetched only when the intent classifier flags a new `store_products` in `required_data`, i.e. when the user's own message asked what they could use or obtain. Every other pre-fetch in that bundle is unconditional precisely so a classifier miss can't blind the model — here a miss is the *desired* failure mode. No catalog in the prompt means the model was never taught the vocabulary, so "Viva never volunteers a product" is a property of what it was handed rather than an instruction it might drift from.
  - **The model picks; the server writes.** The action tail carries a `sku_id` and one sentence of reasoning and nothing else — every name and price the user sees is rendered by `_buildProductCardBlock` from the same snapshot the prompt was built on, exactly as `_buildFormulaChartBlock` already does "so the bars can't disagree with the numbers". Prices are therefore never shown to the model at all: a number it was never given is a number it cannot leak. An unrecognised `sku_id` is dropped silently rather than repaired.
  - **Allergies are filtered in code, before the model sees the catalog.** Any product whose declared allergens/cautions collide with an active `user_memory_facts` allergy or dietary restriction is removed from the prompt entirely, so there is nothing left for the model to get wrong. Matching is deliberately biased toward over-suppression.
  - **Tapping a card row opens the store on that exact item**, reusing the existing `webview_tokens.context` bridge with a new `view_product` intent and GCN's existing `?sku=` deep-link opener — including its silent no-op when the item isn't listed in that buyer's own bound store. A native tap handler is mandatory here, not stylistic: a WeChat miniapp cannot open an external link from chat prose.
  - **PLAN and JUDGE both had to be told what this tail is**, or they would have rejected it as an unsupported claim and burned REVISE rounds — the same failure `remember_fact` hit in §27. JUDGE is told the tail is a control action verified in code, and that the draft's *absence* of a price is correct rather than an omission. A new `detectFakeStoreProduct` covers the remaining gap: a product the model names in prose but never actually picked.


- **AI 精准营养素 — the Viva AG dots ordering flow, end to end** (`worker/lib/agFormulation.js`, `worker/handlers/ag_formulation.js`, `worker/handlers/{viva_ag,boxes,dots,viva_subscription}.js`, `nano-miniapp/pages/main/`, `components/viva-ag-panel/`, 4 nano migrations + GCN `migration_0078`)
  - One purchase in the Aeviva store now runs the whole chain: **buy → AG formulates → a nutrition expert approves → compounded → shipped → the user scans the box and their 28-day plan starts.** Roughly two thirds of it already existed on both sides; this connects them.
  - **Nano now parses and validates the AG formula** (`lib/agFormulation.js`), reversing §8's "nano does not parse or validate this file". That was true while the formula was a read-only artifact; it stopped being true the moment an approved formula gets compounded into physical capsules. Every §8 rule is enforced — 56 capsules present and ordered, no empty capsule, known dot keys, the 72-dot fill limit, each dot's **daily** total inside its own `target_dots_min…max`, non-flexible dots confined to their slot, `DOT-N7` alone on days 10–11 at its max, and the `total_dots` checksum.
  - **It rejects, it never repairs.** A count outside a dot's range is refused, not clamped, and a bad checksum is not recomputed — either would ship a formula nobody authored. The agent's own report is still delivered to the user either way; only the formula is withheld, and `POST /jobs/result` answers with `formulation_accepted` plus the specific violations. `result.formulation` (JSON) is now the contract; the `.md` is a documented fallback.
  - **An approved formula becomes a plan, but not an active one.** Expert approval writes a `nutrition_plans` row at the new `'approved'` status with **no schedules**; the 56 schedule rows are generated when the user scans their delivered box, which is also when `start_date` becomes that day. The 28-day cycle therefore starts when the capsules are in the user's hand, not while they are being compounded and shipped. `handleGetNutritionPlan` already filters `status='active'`, so the previous plan keeps showing until the box arrives — no client change needed.
  - **`_commitAgFormulation` is a deliberate sibling of `_commitNutritionPlan`, not a reuse of it.** That function expands a *steady-state* recipe across the cycle, applying pulse windows, the capsule cap and the `DOT-N7` isolation override day by day. An AG formula already encodes all 56 capsules explicitly, so running it through that expansion would apply every rule twice and flatten the per-day variation the agent deliberately produced. The capsules are written verbatim instead, with the fill cap re-applied defensively only.
  - **Scanning a box is idempotent and non-transferable.** A second scan returns the plan the first one made rather than a second overlapping cycle; a box belonging to someone else is refused outright (`not_your_box`) — these capsules are compounded from one person's biomarkers. Both a bare `WVB…` code and the public ingredient page's URL are accepted, since a WeChat scan can return either.
  - **The AG add-on became purchasable** via a `product_type` column on `viva_subscription_plans`/`_codes` — exactly the upgrade path `migration_users_viva_ag_expiry.sql`'s comment prescribed, so no new endpoint and no new `GCN_ALLOWED_PATHS` entry. A `viva_ag` plan grants **both** windows, because `requireVivaAgAccess()` is a composite of Viva *and* AG; granting only the AG column would leave a paying buyer locked out of the subtab they just bought.
  - **The expert review stays in GCN**, reusing `formulation_reviews`, the `nutrition_expert` role, the claim queue and the inbox fan-out unchanged. Nano exposes the formula through a new `GET /ag-formulation-review-snapshot`, and the twin half of both review snapshots is now one shared `_buildReviewTwinContext` — so the two can't drift into showing reviewers different evidence. The reviewer also gets short-lived signed links to the agent's own report, since the reasoning is most of what there is to review.
  - **An expert's adjustment is re-validated by nano, not trusted.** The reviewer UI can be wrong too, and this recipe is about to become physical capsules — "a human approved it" is not a reason to skip the only check there is. `DOT-N7` is rendered read-only in the expert UI because its dosing is system-controlled, and the dashboard edits **daily** counts (what a dot's min/max actually compares against) then rebuilds the 56 capsules preserving each dot's AM/PM split.
  - **The order waits in a new `awaiting_formulation` status.** The sequence is inverted relative to the existing custom-formulation product: there the recipe existed first and checkout priced it per-dot, here the buyer pays first and the recipe arrives days later. So the bundle is flat-priced, `order_item_custom_formulations.nano_nutrition_plan_id` had to become nullable, and the review row is opened by nano's `formulation-ready` call rather than at payment.
  - **Fixed while testing this live:** the expert dashboard rendered a dot's ingredients as `[object Object]` — nano's `dots.ingredients` is an array of `{mg, name}` objects and the page `join`ed it. Latent until now (a flat review's stored snapshot carries names only, so that line was always empty there); an AG review does carry ingredients, so it surfaced immediately. Now renders `辅酶Q10 15mg、吡咯喹啉醌 5mg`, which is what a reviewer actually needs to judge the mix.
  - Cross-repo notifies are fire-and-forget in both directions, matching the rest of this integration — a formula is safely recorded before GCN hears about it, and an expert's decision is committed before nano hears about it. A `valid` formulation with a NULL `gcn_order_id` is the query that finds the ones that didn't land.
...
- **A replaced formulation's printed label failed instead of saying it was replaced** (`worker/handlers/dots.js`, GCN `site/aeviva/formulation-label.html`)
  - `_getCommittedPlanDay0Breakdown` only read `proposed_recipe` when the status was exactly `'proposed'`. The moment a user re-ran 营养定制, the previous proposal became `'superseded'` — still with no `nutrition_schedules`, because those are only written at box scan — so its label fell through to the schedule lookup and returned `plan_has_no_schedule`. That is the label on a **physical box in someone's hands**, reading as an error because they formulated again.
  - The recipe was on the row the whole time. Day 0 now falls back to `proposed_recipe` whenever a plan has no schedules, regardless of status, and the page renders the existing 已被新配方替代 / Superseded badge plus a QR hint saying scanning it will not activate anything. `handleGetFormulationCheckoutSnapshot` is untouched — a replaced formulation can now be **read**, but must never become purchasable.
  - A code that resolves to nothing at all now says the label has expired and a newer formulation has probably replaced it, rather than "not found" — which read as "your box is fake". It stops short of asserting it, since a mistyped or non-Aeviva code lands there too and the two are indistinguishable.

- **A Formulate-Dots proposal repopulated the Plans tab it was designed to stay out of** (`worker/handlers/{chat,dots}.js`, `nano-miniapp/pages/main/main.js`)
  - The proposal was delivered as `notification_type = 'nutrition_plan'` — the exact type `handleGetNutritionPlan`'s legacy fallback reads as "the plan you are on". So although the `nutrition_plans` row was correctly `'proposed'` and invisible to the structured query, the tab still came back `hasPlan: true` off the notification content. Caught in the WeChat simulator, not by any test: the DB and the API both looked right in isolation.
  - Proposals now deliver as `'formulation_proposal'` (added to `AI_ECHO_TYPES` so the bubble isn't rendered twice), and the legacy fallback is documented as deliberately reading only `'nutrition_plan'` — pre-existing rows only, since nothing writes that type any more.
  - The Plans tab empty state also still promised "complete a Kino test and the system will generate your plan", which is precisely the auto-generation that was just removed. Reworded in both the miniapp and the web app.

- **Nothing creates a nutrition plan on a timer any more** (`dispatcher/index.js`, `worker/index.js`, `worker/handlers/dots.js`)
  - The dispatcher's nutrition top-up scan ran every minute and had **no plan requirement at all** — a LEFT JOIN over `users` counting *schedules*, `HAVING COUNT(s.id) < 7`. It did not top anything up: it manufactured a fresh `active` plan, via an LLM call, for anyone who did not have one. That included every user who had never ordered a box, never received capsules, and had no intention of taking anything.
  - That is how dev accumulated **1,133 plans across 676 users with zero boxes ever produced**, and why the counts kept climbing (2–18/day, matched 1:1 by `nutrition_plan` notifications) as each user's 28-day window dropped below 7 remaining days.
  - Removed end to end so no path can trigger it: the dispatcher scan, the worker's `nutrition.topup` route (a replayed or hand-crafted event would otherwise still reach it), and `handleNutritionTopupEvent` itself. `_commitNutritionPlan` went with it — that handler was its only caller. **A plan is now created by exactly two things, both of which require a scanned box**: `_activateProposedPlan` (a chat-tool proposal) and `_commitAgFormulation` (a Viva AG formula).
  - Two dangling references survived the deletion and would have thrown at runtime — `workerUrl`, still needed by the check-in dispatcher, and `usersToTopUp` in the response body. `node --check` catches neither; both are fixed and the other scans (check-in, user-online, reminders) were confirmed still running in live logs afterwards.

- **A calendar day on the formulation label was reported as the previous day** (`worker/handlers/dots.js`)
  - `started_on` came straight from the `start_date` DATE column, which node-postgres parses at local midnight and then serializes as a UTC instant — a plan starting 2026-08-28 was published as `2026-08-27T16:00:00Z`. Exactly the trap `CLAUDE.md` §35 records. It happened to render correctly on GCN's label page (which formats in Asia/Shanghai) and would have been wrong for every other consumer. Now cast `::text` in SQL, so the day the label states is the day it means.

- **A formulation no longer shrinks every dot below its own minimum to fit the capsule** (`worker/handlers/dots.js`)
  - Each dot has a `target_dots_min`, and those floors sum to more than the 144 dots a day physically holds (2 × 72), so a full formulary genuinely cannot keep them all. `_capRecipeTotal` resolved that by scaling every dot down proportionally — which landed most of them under their own minimum. On the real 18-dot formulary that was **every** formulation: a live check produced 15 dots of which most were sub-therapeutic.
  - Found by pointing `validateAgFormulation` at a real expansion, which rejected it with `dose_below_min` on nearly every dot. Latent until now because nothing validated nano's own output — the AG pipeline validated only formulas authored by the external agent. Adding the fast-track path, where no expert reviews the formula, is what made it load-bearing: **every fast-track submission would have been refused.**
  - **The budget is now settled on daily totals before anything is split into capsules, by dropping whole dots.** A sub-therapeutic dot is worse than an absent one — it occupies capsule space a real dose could have used, and it tells the user they are taking something they effectively are not. The validator agrees: an absent dot is legal, an underdosed one is not.
  - **Which dot goes is read from the formulator's own emphasis**: lowest relative position in its own min–max range first, since that is how emphasis is expressed (see `_fallbackCountForDot`'s 25/50/75%). A dropped dot leaves both slots — half a daily dose is exactly the underdose this prevents. `_capRecipeTotal` still runs afterwards, but on a recipe that already fits it is the no-op safety net it was meant to be.
  - This changes what users get: the same live case went from 15 dots at unusable doses to **8 dots at real ones**. The drop-ranking rule is a product judgement and is worth revisiting with the clinical side.
